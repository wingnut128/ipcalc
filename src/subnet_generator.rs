...
pub fn generate_ipv4_subnets(cidr: &str, new_prefix: u8, count: u64) -> Result<Ipv4SubnetList> {
    ...
    let subnets: Result<Vec<Ipv4Subnet>> = (0..count)
        .map(|i| {
...
pub fn generate_ipv6_subnets(cidr: &str, new_prefix: u8, count: u64) -> Result<Ipv6SubnetList> {
    ...
    let subnets: Result<Vec<Ipv6Subnet>> = (0..count)
        .map(|i| {
...
mod tests {
    ...
    fn test_generate_ipv4_subnets() {
        let result = generate_ipv4_subnets("192.168.0.0/22", 27, 10).unwrap();
    ...
    fn test_generate_ipv4_subnets_max() {
        ...
        let result = generate_ipv4_subnets("192.168.0.0/22", 27, 32).unwrap();
    ...
    fn test_generate_ipv4_subnets_too_many() {
        ...
        let result = generate_ipv4_subnets("192.168.0.0/22", 27, 33);
        assert!(result.is_err());
    }

    #[test]
    fn test_generate_ipv6_subnets() {
        let result = generate_ipv6_subnets("2001:db8::/32", 48, 5).unwrap();
    ...
    fn test_invalid_new_prefix_smaller() {
        let result = generate_ipv4_subnets("192.168.0.0/24", 22, 1);
        assert!(result.is_err());
    }
}
